...
public class YamlPropertySourceFactory implements PropertySourceFactory {
	...
	public PropertySource<?> createPropertySource(@Nullable String name, EncodedResource encodedResource)
		...
		String defaultFileName = StringUtils.defaultIfEmpty(encodedResource.getResource().getFilename(),
				String.format("file-%s", UUID.randomUUID()));
		String sourceName = StringUtils.defaultIfEmpty(name, defaultFileName);

		return new PropertiesPropertySource(sourceName, properties);
	}

	private static Properties loadYamlIntoProperties(EncodedResource resource) throws FileNotFoundException {
		try {
